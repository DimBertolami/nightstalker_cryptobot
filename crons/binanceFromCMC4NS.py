...
def main():
    ...
    try:
        binance_exchange_id = get_exchange_id(cursor, 'binance')
        ...
        for symbol_data in binance_data['symbols']:
            ...
            if base_asset not in ['EUR', 'USD', 'USDT', 'BUSD']:
                binance_symbols.add(base_asset)
        print(f"Found {len(binance_symbols)} unique symbols on Binance.")

        # 2. Fetch data for these symbols from CMC
        cmc_data = fetch_cmc_data_for_symbols(list(binance_symbols), headers)
        ...
        if first_coin_exchange_id == 1:  # If the coins are from Bitvavo
            print("Data is from Bitvavo, purging existing coin data...")
        ...
        for coin in matched_coins:
            ...
            values = (
                ...
                last_updated, date_added, binance_exchange_id
            )
