...
def get_bitvavo_coins_from_cmc():
    ...
    """
    # Your CoinMarketCap API key (2) : 'e2e746c1-169a-4778-90f7-a66458a6af00'
    ...
    try:
        ...
        # Extract all unique base currencies (coins) from Bitvavo
        bitvavo_coins = set()
        ...
        # Now, get a list of all cryptocurrencies from CMC for additional details
        ...
        # Match Bitvavo coins with our listings data
        coin_details = []
        matched_count = 0
...
if __name__ == "__main__":
    ...
    if bitvavo_coins is not None:
        ...
        csv_filename = f'bitvavo_coins_{timestamp}.csv'
        bitvavo_coins.to_csv(csv_filename, index=False)
        print(f"\nSaved complete list of {len(bitvavo_coins)} coins to {csv_filename}")
        ...
        if choice == "1":
            print("\nFirst 10 coins:")
            print(bitvavo_coins.head(10))
        elif choice == "2":
            print("\nDisplaying complete list with pagination:")
            display_paginated_dataframe(bitvavo_coins, 100)