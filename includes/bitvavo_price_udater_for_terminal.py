...
script_handler = logging.FileHandler('/opt/lampp/htdocs/NS/logs/bitvavo_script_terminal.log') # Changed log file
...
price_handler = logging.FileHandler('/opt/lampp/htdocs/NS/logs/price_updates_terminal.log') # Changed log file
    ...
    def log_and_save_portfolio_prices(self):
        ...
        try:
            ...
            for symbol_eur in symbols_to_track:
                ...
                print(f"{symbol_eur}:{current_price}") # Keep terminal output
                ...
                if apex_data:
                    ...
                    if status == 'sold':
                        # If already sold, skip all processing for this coin
                        script_logger.info(f"Coin {symbol} is marked as 'sold'. Skipping further processing.")
                        continue
...
if __name__ == '__main__':
    ...
    while running:
        if not bvavo.log_and_save_portfolio_prices():
            script_logger.info("No active portfolio items. Stopping script.")
            break
        time.sleep(3)
