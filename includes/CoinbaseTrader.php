...
class CoinbaseTrader {
    ...
    public function __construct($testMode = true) {
        ...
        $this->exchange = new coinbasepro([
            'apiKey' => '', // Set dynamically from config
            'secret' => '', // Set dynamically from config
            'password' => '', // Coinbase may require passphrase; set dynamically
            'enableRateLimit' => true,
        ]);
        if ($testMode) {
            // Coinbase may not support sandbox; check CCXT docs
