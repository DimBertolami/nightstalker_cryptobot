...
// Handle default exchange form submission
$message = '';
$messageType = '';
if (
    isset($_POST['default_exchange']) 
    && in_array($_POST['default_exchange'], ['binance', 'bitvavo'])
) {
    $_SESSION['default_exchange'] = $_POST['default_exchange'];
    $message = 'Default exchange updated!';
    $messageType = 'success';
}


// Get current settings
$configFile = realpath(__DIR__ . '/../crons/execute_new_coin_strategy.php');
if ($configFile === false) {
    $message = "Config file not found! (Path resolution failed for <code>" . __DIR__ . "/../crons/execute_new_coin_strategy.php</code>)";
    $messageType = "danger";
    error_log("[settings.php] Config file path resolution failed: " . __DIR__ . '/../crons/execute_new_coin_strategy.php');
}

// Get current default exchange (default to binance)
$currentDefaultExchange = $_SESSION['default_exchange'] ?? 'binance';

// Show success message if set
if ($messageType === 'success') {
    echo "<div class='alert alert-success'>{$message}</div>";
    // Log to browser console if default exchange was changed
    if (isset($_POST['defaultExchange']) || isset($_POST['default_exchange'])) {
        echo "<script>console.log('Default exchange updated: ' + '" . htmlspecialchars($_SESSION['default_exchange'] ?? '') . "');</script>";
    }
}
        ...
        if (empty($errors)) {
            ...
            if (file_exists($configFile)) {
                ...
                if (file_put_contents($configFile, $configContent)) {
                    ...
                    $message = "Failed to write settings to file. <br>Please check file and directory permissions for <code>$configFile</code>.";
                    $messageType = "danger";
                }
            } else {
