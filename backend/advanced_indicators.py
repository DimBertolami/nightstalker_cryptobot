...
class AdvancedIndicators:
    ...
    def _calculate_momentum_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            williams_r = ta.momentum.WilliamsRIndicator(high=df['High'], low=df['Low'], close=df['Close'])
            df['Williams_R'] = williams_r.williams_r()
            
            # Calculate Ultimate Oscillator
            ultimate_osc = ta.momentum.UltimateOscillator(high=df['High'], low=df['Low'], close=df['Close'])
    ...
    def _calculate_sentiment_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            cmf = ta.volume.AccDistIndexIndicator(high=df['High'], low=df['Low'], close=df['Close'], volume=df['Volume'])
            df['CMF'] = cmf.acc_dist_index()
