...
// Create a backup
fs.writeFileSync(`${filePath}.final_backup2`, content);
console.log(`Created backup at ${filePath}.final_backup2`);

// Fix the buy button click handler and processCoinData function
let fixed = content;

// Fix the buy button click handler
fixed = fixed.replace(
  /\/\/ Buy button click handler[\s\S]*?complete: function\(\) \{[\s\S]*?\$button\.html\('Buy'\);[\s\S]*?\}\s*\}\s*\}\s*\)\s*;\s*\}\s*\}\s*\);/,
  `// Buy button click handler - update the existing handler
...
fixed = fixed.replace(
  /function processCoinData\(data\) \{[\s\S]*?\$\('#coins-table tbody'\)\.append\(rowHtml\);[\s\S]*?\}\s*\}\s*\}\s*\}/,
