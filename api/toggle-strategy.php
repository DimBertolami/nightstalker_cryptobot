<?php
/**
 * Toggle Strategy API Endpoint
 * 
 * Activates or deactivates a trading strategy for the Night Stalker cryptobot
 */
...
try {
    ...
    if (!isset($_POST['is_active'])) {
        throw new Exception('Active status is required');
    }
    ...
    $stmt = $db->prepare("UPDATE strategies SET is_active = :is_active, updated_at = CURRENT_TIMESTAMP WHERE id = :id");
    $stmt->bindParam(':id', $id);
    ...
    echo json_encode([
        'success' => true,
        'message' => "Strategy '{$strategy['name']}' has been $statusText",
        'is_active' => $isActive
    ]);
